### SOURCE: https://github.com/NVlabs/NVAE/blob/master/utils.py
...
class Writer(object):
    ...
    def add_image(self, text, image, step):
        if self.rank == 0:
            self.writer.add_image(text, image, step)
...
def log_iw(decoder, x, log_q, log_p, crop=False):
    recon = reconstruction_loss(decoder, x)
...
# def tile_image_1d(batch_image, n):
#     # assert n * n == batch_image.size(0)
#     len, width = batch_image.size(1), batch_image.size(2)
#     batch_image = batch_image.view(n, channels, width)
#     batch_image = batch_image.permute(1, 0, 2)                              # n, height, n, width, c
#     batch_image = batch_image.contiguous().view(channels, n * width)
#     return batch_image 
