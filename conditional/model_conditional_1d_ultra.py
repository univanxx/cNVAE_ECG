...
class AutoEncoder(nn.Module):
    def __init__(self, args, writer, arch_instance, num_classes):
        ...
        # self.embedding = nn.Parameter(torch.rand(size=(self.num_classes,)+prior_ftr0_size), requires_grad=True) # For classes
        ...
        if self.add_label:
            self.embed_transform = nn.Conv1d(768, 12, kernel_size=40)
    ...
    def sample(self, idx, num_samples, t):
        ...
        class_one = idx
        classes = torch.zeros(self.num_classes).bool()
        classes[class_one] = True
        s = (self.prior_ftr0 * classes.cuda().unsqueeze(1).unsqueeze(2)).sum(axis=0)
