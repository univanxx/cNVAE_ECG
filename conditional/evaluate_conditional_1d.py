...
def set_bn(model, idx, bn_eval_mode, num_samples=1, t=1.0, iter=100):
    if bn_eval_mode:
        model.eval()
    else:
        model.train()
        with autocast():
            for i in range(iter):
                ...
                model.sample(idx, num_samples, t)
        model.eval()
...
def main(eval_args):
    ...
    if eval_args.readjust_bn:
        logging = utils.Logger(eval_args.local_rank, eval_args.save+'_temp_'+str(eval_args.temp)+'_type_'+str(eval_args.class_type))
    else:
        logging = utils.Logger(eval_args.local_rank, eval_args.save+'_temp_'+str(eval_args.temp)+'_type_'+str(eval_args.class_type)+'_no_bn')
    # load a checkpoint
    ...
    # print(eval_args.num_input_channels)
    ...
    if eval_args.eval_mode == 'evaluate':
        ...
        set_bn(model, eval_args.class_type, bn_eval_mode, num_samples=2, t=eval_args.temp, iter=eval_args.num_iters)
        ...
        num_iter = 71 if eval_args.class_type == 1 else 172

        with torch.no_grad():
            n = int(np.floor(np.sqrt(num_samples)))
            set_bn(model, eval_args.class_type, bn_eval_mode, num_samples=16, t=eval_args.temp, iter=eval_args.num_iters)
            for ind in np.arange(num_iter):     # sampling is repeated.
                if os.path.exists(os.path.join(eval_args.save+ '_temp_'+str(eval_args.temp)+ '_type_'+str(eval_args.class_type), 'bn_' + str(eval_args.num_iters), 'gpu_%d_samples_%d.npz' % (eval_args.local_rank, ind))):
                    print("%s skipped!" % ind)
                    continue
                else:
                    ...
                    start = time()
                    with autocast():
                        logits = model.sample(eval_args.class_type, num_samples, eval_args.temp)

                    output = model.decoder_output(logits)
                    if args.num_input_channels == 8:
                        output_img = output.mean if isinstance(output, torch.distributions.bernoulli.Bernoulli) else output.sample(eval_args.temp)
                        output_tiled = output_img.cpu()
                    else:
                    ...
                    visualize = False
                    if visualize:
                        output_tiled = utils.tile_image(output_img, n).cpu().numpy()
                        ...
                        if eval_args.readjust_bn:
                            dir_path = eval_args.save+ '_temp_'+str(eval_args.temp)+ '_type_'+str(eval_args.class_type) + '/' + 'bn_' + str(eval_args.num_iters)
                            Path(dir_path).mkdir(parents=True, exist_ok=True)
                            file_path = os.path.join(eval_args.save+ '_temp_'+str(eval_args.temp)+ '_type_'+str(eval_args.class_type), 'bn_' + str(eval_args.num_iters), 'gpu_%d_samples_%d.npz' % (eval_args.local_rank, ind))
                            np.savez_compressed(file_path, samples=output_tiled)
                        else:
                            dir_path = eval_args.save+ '_temp_'+str(eval_args.temp)+ '_type_'+str(eval_args.class_type) + '/no_bn'
                            Path(dir_path).mkdir(parents=True, exist_ok=True)
                            file_path = os.path.join(eval_args.save+'_temp_'+str(eval_args.temp) +'_type_'+str(eval_args.class_type), 'no_bn', 'gpu_%d_samples_%d.npz' % (eval_args.local_rank, ind))
                            np.savez_compressed(file_path, samples=output_tiled)
...
if __name__ == '__main__':
    ...
    # ECG
    parser.add_argument('--class_type', type=int, help='Class type: 0 or 1', required=True)
    ...
    if args.readjust_bn:
        utils.create_exp_dir(args.save+'_temp_'+str(args.temp)+'_type_'+str(args.class_type))
    else:
        utils.create_exp_dir(args.save+'_temp_'+str(args.temp)+'_type_'+str(args.class_type)+'_no_bn')
